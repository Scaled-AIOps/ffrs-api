...
import { aggregate, collectItems, STUDY_START, toExportRow } from '../src/domain/metrics.js';
...
const items = (await collectItems(githubTracker(repo, token))).filter((i) => i.createdAt >= STUDY_START);
