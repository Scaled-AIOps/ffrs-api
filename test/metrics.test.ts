...
describe('metrics from GitHub', () => {
  ...
  it('collectItems + aggregate: kind from labels, respond from first human comment, spam via label, report issues ignored', async () => {
    ...
    expect(items[0]).toMatchObject({ severity: 'high', outcome: 'fixed', spam: false });
    ...
    expect(m[0]).toMatchObject({ ttfrP50: 20 * 3600, ttcP50: 48 * 3600, loopClosure: 0.5, signalRatio: 0.5 });
    expect(m[1]).toMatchObject({ ttfrP50: null, loopClosure: 0, signalRatio: 1 });
  ...
  it('weekly report renders and is filed as an issue', async () => {
    const rows = [{ kind: 'bug' as const, week: '2026-08-10', n: 3, ttfrP50: 20 * 3600, ttfrP90: 60 * 3600, ttcP50: 3 * 86400, loopClosure: 1, signalRatio: 0.67 }];
    const r = weeklyReport(rows, '2026-08-10', 'scaledaiops.org');
    expect(r.body).toContain('| 2026-08-10 | bug | 3 | 20.0 h | 2.5 d | 3.0 d | 100% | 67% |');
