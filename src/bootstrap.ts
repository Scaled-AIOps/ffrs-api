 ...
 */
const SSM_BACKED = ['GITHUB_TOKEN', 'GITHUB_WEBHOOK_SECRET', 'TURNSTILE_SECRET'] as const;
...
export async function loadSecretsFromSsm(prefix: string, env: NodeJS.ProcessEnv = process.env): Promise<void> {
  ...
  // turnstile/webhook secrets may legitimately be absent; GITHUB_TOKEN is enforced by config.
}
